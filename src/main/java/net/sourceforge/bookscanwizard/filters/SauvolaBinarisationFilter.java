...
/**
 * Performs local image thresholding with Sauvola algorithm.
 ...
 */
public final class SauvolaBinarisationFilter extends MedianFilter {
    ...
    private final SquaredIntergalImageFilter squaredIntergalImageFilter;
    private final Image squaresImage;
    ...
    public static RenderedImage filter(RenderedImage img) {
        if (img.getSampleModel().getSampleSize(0) > 1) {
            if (img.getColorModel().getNumColorComponents() > 1) {
                img = Color.toGray(img);
            }
            PixelImage sourceImage = new PixelImage(Color.toGray(img));
            PixelImage destinationImage = new PixelImage(img.getWidth(), img.getHeight());
            SauvolaBinarisationFilter filter = new SauvolaBinarisationFilter(255,0,destinationImage,255,.2,15);
            filter.process(sourceImage);
            img = destinationImage.toBufferedImage();
        }
        return img;
    }
}
